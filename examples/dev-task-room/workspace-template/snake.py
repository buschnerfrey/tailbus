...
def _draw(stdscr: curses.window, game: SnakeGame) -> None:
    ...
    if max_y < min_height or max_x < min_width:
        ...
        _safe_addstr(stdscr, max(0, max_y // 2 - 1), max(0, (max_x - len(resize_msg)) // 2), resize_msg)
        _safe_addstr(stdscr, max(1, max_y // 2), max(0, (max_x - len(size_msg)) // 2), size_msg)
        stdscr.refresh()
        return
    ...
    _safe_addstr(stdscr, instructions_y, board_x, "Arrows/WASD to move. R to restart, Q to quit.")
    if not game.alive:
